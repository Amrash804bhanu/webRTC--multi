...
📄 Appendix: Project Design Report
...
1-minute Loom Video: A hosted video link (to be added here: [Your Loom Video Hosted Link Here]) demonstrating:

Phone → browser live overlay of detections.
...
WASM Mode: Performs inference directly on the client-side using onnxruntime-web, providing a low-resource path runnable on modest laptops (no GPU required). This mode includes quantized small models, input downscaling to 320x240, and adaptive sampling (10–15 FPS).
